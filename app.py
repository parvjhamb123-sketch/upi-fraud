...
return (train_npz['X_train'], train_npz['y_train'],
...
with col2:
    ...
    if clicked_vals:
        amount       = clicked_vals[0]; old_bal_orig = clicked_vals[1]
        new_bal_orig = clicked_vals[2]; old_bal_dest = clicked_vals[3]
        new_bal_dest = clicked_vals[4]; txn_type     = clicked_vals[5]
...
with col_results:
    ...
    if train_btn:
        ...
        X_tr = X_train_ready[:val_split]; y_tr = y_train_ready[:val_split]
        X_vl = X_train_ready[val_split:]; y_vl = y_train_ready[val_split:]

        for epoch in range(epochs):
            h = tuned_model.fit(X_tr, y_tr, epochs=1, batch_size=batch_size,
                                    validation_data=(X_vl,y_vl), verbose=0)
            tl=h.history['loss'][0]; vl=h.history['val_loss'][0]
            tau=h.history['auc'][0]; vau=h.history['val_auc'][0]
            train_losses.append(tl); val_losses.append(vl)
            train_aucs.append(tau);  val_aucs.append(vau)

            if vau > best_val_auc:
                best_val_auc=vau; patience_counter=0
            ...
            if (epoch+1) % 2 == 0 or epoch == 0:
                ...
                fig_live.update_layout(height=260,legend=dict(orientation='h',y=1.15),**PLOTLY_BASE)
        ...
        try:    tuned_model.load_weights('/tmp/best_tuned.weights.h5')
        except: pass
    ...
    if 'tuned_metrics' in st.session_state:
        ...
        st.markdown('<div class="section-title">🎚️ Threshold Sensitivity</div>',
                    unsafe_allow_html=True)
        y_pp2    = st.session_state['tuned_keras_model'].predict(
                    X_test_ready,verbose=0).flatten()
...
fig=go.Figure(); MAX_D=12
for i,(name,n,color) in enumerate(layer_data):
    d=min(n,MAX_D)
    for j in range(d):
        fig.add_trace(go.Scatter(x=[i],y=[j-d/2],mode='markers',
...
for i in range(len(layer_data)-1):
    d1=min(layer_data[i][1],MAX_D); d2=min(layer_data[i+1][1],MAX_D)
...
with col2:
    ...
    fig=go.Figure(go.Bar(x=list(range(24)),y=fraud_h,
