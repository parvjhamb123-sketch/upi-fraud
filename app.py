...
/* Inputs */
.stSelectbox > div, .stNumberInput > div { background: #0d1424 !important; }
div[data-testid="stNumberInput"] input { background: #0d1424 !important; color: #e2e8f0 !important; }
...
/* Plotly chart backgrounds consistent */
...
# ══════════════════════════════════════════════════════════════════
# LOAD ARTIFACTS (ONNX — fast loading)
# ══════════════════════════════════════════════════════════════════
@st.cache_resource
def load_artifacts():
    session = ort.InferenceSession('model/ann_model.onnx')
...
def load_train_test_data():
    test_npz      = np.load('model/test_data.npz')
    train_npz     = np.load('model/train_data.npz')
    X_test_ready  = test_npz['X_test']
    y_test_ready  = test_npz['y_test']
    X_train_ready = train_npz['X_train']
    y_train_ready = train_npz['y_train']
    return X_train_ready, y_train_ready, X_test_ready, y_test_ready
...
def onnx_predict(sess, feat_scaled):
    ...
    return sess.run([output_name], {input_name: feat_scaled.astype(np.float32)})[0][0][0]
...
if page == "🏠 Dashboard":
    ...
    <div class="main-header">
        ...
        <p>Real-time AI-powered transaction security · Deep Learning ANN Model · PaySim Dataset</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        fig.update_layout(title='📈 AUC Curve', height=300,
                          legend=dict(orientation='h', y=1.12),
                          xaxis_title='Epoch',
                          yaxis=dict(range=[0.5, 1], title='AUC'),
                          **PLOTLY_BASE)
        st.plotly_chart(fig, use_container_width=True)
...
# ══════════════════════════════════════════════════════════════════
    ...
    with col1:
        ...
        c1, c2 = st.columns(2)
        old_bal_orig = c1.number_input("Balance Before (₹)", min_value=0.0, value=100000.0, step=1000.0, key='ob_o')
        new_bal_orig = c2.number_input("Balance After (₹)",  min_value=0.0, value=50000.0,  step=1000.0, key='nb_o')
        st.markdown("**Receiver Account**")
        c3, c4 = st.columns(2)
        old_bal_dest = c3.number_input("Balance Before (₹)", min_value=0.0, value=5000.0,  step=1000.0, key='ob_d')
        new_bal_dest = c4.number_input("Balance After (₹)",  min_value=0.0, value=55000.0, step=1000.0, key='nb_d')
        ...
        for label, vals in scenarios.items():
            if st.button(label, use_container_width=True, key=f"sc_{label}"):
                clicked_scenario = vals

    with col2:
        st.markdown('<div class="section-title">📊 Analysis Result</div>', unsafe_allow_html=True)

        run_analysis = analyze or (clicked_scenario is not None)
        if clicked_scenario:
            amount, old_bal_orig, new_bal_orig = clicked_scenario[0], clicked_scenario[1], clicked_scenario[2]
            old_bal_dest, new_bal_dest, txn_type = clicked_scenario[3], clicked_scenario[4], clicked_scenario[5]
        ...
        if run_analysis:
            ...
            if prob >= threshold:
                ...
                <div class="{css}">
                    ...
                    <div style="font-family:'Syne',sans-serif;font-size:1.4rem;font-weight:800;color:{color};margin:6px 0">FRAUD DETECTED</div>
                    <div style="font-size:3rem;font-weight:800;color:white;font-family:'Syne',sans-serif">{prob*100:.1f}%</div>
                    <div style="color:#94a3b8;font-size:0.85rem">{risk} · Threshold: {threshold:.2f}</div>
                </div>""", unsafe_allow_html=True)
            else:
                st.markdown(f"""
                <div class="safe-alert">
                    <div style="font-size:2rem">✅</div>
                    <div style="font-family:'Syne',sans-serif;font-size:1.4rem;font-weight:800;color:#22c55e;margin:6px 0">TRANSACTION SAFE</div>
                    <div style="font-size:3rem;font-weight:800;color:white;font-family:'Syne',sans-serif">{prob*100:.1f}%</div>
                    <div style="color:#94a3b8;font-size:0.85rem">LOW RISK · Threshold: {threshold:.2f}</div>
                </div>""", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════
    ...
    <div class="main-header">
        ...
        <p>Customize the ANN architecture · Train live · Compare with base model in real-time</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_results:
        ...
        if train_btn:
            ...
            for epoch in range(epochs):
                h = tuned_model.fit(X_tr, y_tr, epochs=1,
                ...
                train_aucs.append(tau);   val_aucs.append(vau)
                ...
                if (epoch + 1) % 2 == 0 or epoch == 0:
                    ...
                    fig_live.add_trace(go.Scatter(x=ep_list, y=train_aucs, name='Train AUC',
                                                   line=dict(color='#34d399', width=2), showlegend=False), row=1, col=2)
                    fig_live.add_trace(go.Scatter(x=ep_list, y=val_aucs, name='Val AUC',
                                                   line=dict(color='#fbbf24', width=2, dash='dot'), showlegend=False), row=1, col=2)
                    fig_live.update_layout(height=260, legend=dict(orientation='h', y=1.15),
                                           **PLOTLY_BASE)
        ...
        if 'tuned_metrics' in st.session_state:
            ...
            for col, (name, bv, tv, hb) in zip([c1,c2,c3,c4], compare_data):
                ...
                <div class="metric-card">
                    ...
                    <div style="color:#64748b;font-size:0.78rem;margin:4px 0">Base: <b style="color:#94a3b8">{bv:.4f}</b></div>
            ...
            bv   = [bm['roc_auc'], float(bm['report']['Fraud']['precision']),
                    float(bm['report']['Fraud']['recall']),
                    float(bm['report']['Fraud']['f1-score']),
                    float(bm['report']['accuracy'])]
            tv   = [tm['roc_auc'], tm['precision'], tm['recall'],
                    tm['f1'], tm['accuracy']]
            fig  = go.Figure()
            fig.add_trace(go.Scatterpolar(r=bv+[bv[0]], theta=cats+[cats[0]],
                                           fill='toself', name='Base',
                                           fillcolor='rgba(129,140,248,0.12)',
                                           line=dict(color='#818cf8', width=2)))
            fig.add_trace(go.Scatterpolar(r=tv+[tv[0]], theta=cats+[cats[0]],
                                           fill='toself', name='Tuned',
                                           fillcolor='rgba(192,132,252,0.12)',
                                           line=dict(color='#c084fc', width=2)))
            fig.update_layout(title='🕸️ Radar: Base vs Tuned', height=360,
            ...
            st.markdown('<div class="section-title">🎚️ Threshold Sensitivity</div>',
                        unsafe_allow_html=True)
            tm_model = st.session_state['tuned_keras_model']
            y_pp2    = tm_model.predict(X_test_ready, verbose=0).flatten()
            ...
            fig.add_vline(x=threshold, line_dash='dash', line_color='white',
                          opacity=0.6, annotation_text=f"Chosen: {threshold:.2f}",
                          annotation_position="top right")
            fig.update_layout(title='Impact of Threshold on All Metrics', height=340,
            ...
            **Neurons** — More neurons = more capacity. Use funnel shape (256→128→64) for best results.

            **Dropout** — Prevents overfitting. Keep between 0.2–0.5.

            **Learning Rate** — 0.001 is the sweet spot. Too high = unstable, too low = slow.

            **Decision Threshold** — Most impactful business lever:
            - 🔽 Lower (0.3) → Catch more fraud, but more false alarms on legit transactions
...
# ══════════════════════════════════════════════════════════════════
    ...
    for i, (name, n, color) in enumerate(layer_data):
        ...
        fig.add_annotation(x=i, y=-(MAX_D/2 + 2), text=name,
        ...
        for j1 in range(min(d1, 5)):
            for j2 in range(min(d2, 5)):
                fig.add_shape(type='line',
                    x0=i, y0=j1-d1/2, x1=i+1, y1=j2-d2/2,
...
# ══════════════════════════════════════════════════════════════════
    ...
    with col2:
        ...
        fig = go.Figure()
        fig.add_trace(go.Bar(x=months, y=prevented, name='Fraud Prevented (₹Cr)',
                              marker_color='#34d399'))
        fig.add_trace(go.Bar(x=months, y=missed, name='Missed Fraud (₹Cr)',
                              marker_color='#f87171'))
    ...
    with c3:
        st.error("🔄 **Zero-Balance Pattern**\n\nIf sender balance hits ₹0 after a TRANSFER, fraud probability exceeds 85%. Recommend auto-block.")
